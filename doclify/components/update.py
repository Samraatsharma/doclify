...
def update_docs(path, model=None, provider=None, base_dir: Path = None, progress_callback=None, auto_confirm_readme: bool = False):
    """
    ...
    logger.info(f"Update sequence triggered for path: {path}. Directory: {root}, Overrides: model={model}, provider={provider}")
    ...
    if config_path.exists():
        try:
            ...
            report_progress("error", 1.0, f"Error reading doclify.yaml: {e}")
            return False
    else:
        config = {}
    ...
    if not files_to_process:
        ...
        report_progress("error", 1.0, "No files found to update")
        return False
    ...
    # 1. Reading Phase
    report_progress("reading", 0.20, f"Reading {len(files_to_process)} files...")
    ...
    if not all_file_contents:
        ...
        report_progress("error", 1.0, "No valid file content found to update")
        return False
    ...
    with console.status(f"[bold cyan]Processing[/bold cyan] Files (0/{total_files})...", spinner="dots") as status:
        ...
        for item in all_file_contents:
            ...
            if summary:
                ...
                expected_filename = os.path.normpath(file_path).replace("\\", "/")
            ...
            report_progress(
                "processing", 
                0.3 + (0.5 * (processed_count / max(total_files, 1))), 
                f"Updated summary for {file_path}",
    ...
    should_regen = False
    if auto_confirm_readme:
        should_regen = True
    else:
        logger.info("Prompting user for README regeneration.")
        try:
            should_regen = Confirm.ask("Regenerate README with latest changes?")
        except Exception:
            should_regen = False

    if should_regen:
        logger.info("Regenerating README with latest changes.")
        report_progress("synthesizing", 0.90, "Regenerating README...")
        ...
        report_progress("complete", 1.0, f"README regenerated in {duration:.1f}s", readme=final_readme)
    else:
        logger.info("README regeneration skipped.")
        report_progress("complete", 1.0, "Cache updated without README regeneration.")
