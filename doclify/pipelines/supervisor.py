...
@cli.command()
@click.option('--port', default=8000, help='Port to bind the server on.')
@click.option('--host', default='127.0.0.1', help='Host to bind the server on.')
@click.option('--open-browser/--no-open-browser', default=True, help='Open default browser automatically.')
def server(port, host, open_browser):
    """
    Launch the Doclify Web Platform & API Server.
    
    Starts the local FastAPI backend and serves the interactive Verdant-styled web application.
    """
    import webbrowser
    from doclify.server import start_server
    
    url = f"http://{host}:{port}"
    console.print(f"\n[bold green]✦ Launching Doclify Web Platform[/bold green] at [cyan]{url}[/cyan]\n")
    if open_browser:
        try:
            webbrowser.open(url)
        except Exception:
